...
ball_img: Union[Surface, SurfaceType] = pygame.image.load("anyrgb.com (1).png") #Load once before the game loop
ball_img = pygame.transform.scale(ball_img, [80,55])
if not ball_img:
...
def ball_movement():
    ...
    if ball.colliderect(player):
        if abs(ball.bottom - player.top):  # Check if ball hits the top of the paddle
            # TODO Task 2: Fix score to increase by 1
            score = score +1# Increase player score
            ball_speed_y *= -1  # Reverse ball's vertical direction
            # TODO Task 3: Increase the ball's speed by x
            ball_speed_x -=0.5 #Increases the ball's
            ball_speed_y -=0.5 #speed after every hit
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYUP:
            ...
            if event.key == pygame.K_ESCAPE:
                pygame.quit()
                sys.exit() #Exit game with ESC key
    ...
    screen.blit(ball_img, ball.topleft)  # Draw ball
